...
# Get data structures
with open("mw_data_response_pbe.yaml") as f, open("datafiles_orca_response_v2.yaml") as g, open("mw_data_0001v2.yaml") as h:
    data2 = yaml.load(f)
    data1 = yaml.load(g)
    data3 = yaml.load(h)
...
re_filter = False
molecules = [mol for mol in functions.incommon(data2, data1) if mol not in skip]
...
rel_err_mw_gto = [100 * (data1[mol]["mean"] / data2[mol]["pbe"]["mean"] - 1) for mol in molecules]
...
spin_colors = ["deepskyblue" if data3[mol]["multiplicity"] == 1 else "crimson" for mol in molecules_sorted]
...
# Title
plt.title("GTO Response   vs   MW Response", fontsize=fontsize+2)
